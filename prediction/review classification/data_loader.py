...
class TextDS:
    def __init__(self, path, preprocessor=Preprocessor(), num_terms=5000, batch_size=__settings__.batch_size, split=__settings__.split, window_size=__settings__.window_size, dev='cpu'):
        """
    ...
    def batch_method(self, batch):
        ...
        for i in range(len(sentences)):
            for j in range(w, len(sentences[i])-w):
                temp_list = []
                y.append(sentences[i][j])
                temp_list = sentences[i][j-w:j]+ sentences[i][j+1:j+w+1]
                x.append(temp_list)
        return x, y
...
class LabelledTextDS(TextDS):
    ...
    def get_vector_representation(self):
        ...
        vectorizer = CountVectorizer(lowercase=False,
                                     ...
                                     stop_words=None,
                                     #max_features=50,  ## This only keeps the 100 most frequent words as features/ 可改
